...
CREATE TYPE return_step_type AS (
    id UUID
  , step_type step_type
  , value TEXT
  , docstring TEXT
...
CREATE OR REPLACE FUNCTION create_or_replace_step (
    _id           UUID             -- id          (1)
  , _type         step_type   -- step_type   (2)
  , _value        TEXT             -- value       (3)
  , _docstring    TEXT             -- docstring   (4)
) RETURNS return_step_type
AS $$
DECLARE
  res return_step_type;
  v_state   TEXT;
  v_msg     TEXT;
  v_detail  TEXT;
  v_hint    TEXT;
...
BEGIN
  ...
  ON CONFLICT (id) DO
    ...
    SET   step_type  = EXCLUDED.step_type
        ...
        , updated_at = NOW();
  SELECT id, step_type, value, docstring, created_at, updated_at FROM steps WHERE id = $1 INTO res;
  RETURN res;
...
CREATE OR REPLACE FUNCTION add_step_to_scenario (
...
CREATE OR REPLACE FUNCTION add_step_to_background (
...
BEGIN
  -- Relying on foreign key constraints to ensure scenario id and step id exists
  INSERT INTO background_step_map
  VALUES (
      $1 -- background id
    , $2 -- step id
  );
