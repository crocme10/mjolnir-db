...
CREATE TYPE return_feature_type AS (
    id UUID
  , name TEXT
  , description TEXT
  , tags TEXT[]
...
BEGIN
  ...
  FROM features AS f, websearch_to_tsquery($1) AS query
...
CREATE OR REPLACE FUNCTION create_or_replace_feature (
  ...
  , _tags        TEXT[]    -- tags        (3)
) RETURNS return_feature_type
AS $$
DECLARE
  res return_feature_type;
  v_state   TEXT;
  v_msg     TEXT;
  v_detail  TEXT;
  v_hint    TEXT;
  v_context TEXT;
BEGIN
  INSERT INTO features (name, description, tags) VALUES (
      $1  -- name
    , $2  -- description
    , $3  -- tags
  )
